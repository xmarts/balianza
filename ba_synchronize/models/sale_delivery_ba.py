...
class SaleDeliveryBa(models.Model):
	...
	amount_by_group = fields.Binary(string="Impuestos", compute='_amount_by_group', help="type: [(name, amount, base, formated amount, formated base)]")
	...
	delivery_stock_ids = fields.One2many('sale.delivery.available.ba','delivery_id',string="Puntos de venta disponibles")
...
class SaleDeliveryLineBa(models.Model):
	...
	currency_id = fields.Many2one(related='delivery_id.currency_id', depends=['delivery_id.currency_id'], store=True, string='Currency', readonly=True)

	@api.depends('product_uom_qty', 'discount', 'price_unit', 'tax_id')
	def _compute_amount(self):
		for line in self:
			price = line.price_unit * (1 - (line.discount or 0.0) / 100.0)
			taxes = line.tax_id.compute_all(price, line.delivery_id.currency_id, line.product_uom_qty, product=line.product_id, partner=line.delivery_id.partner_id)
			line.update({
